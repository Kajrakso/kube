...
void fix_eo_lr_ud(cube_t* cube) {
    ...
    for (i = 0; i < NEDGES; i++)
    {
        ...
        else if ((e_slice(i) == M && e_slice(perm) == E) || (e_slice(i) == E && e_slice(perm) == M))
        {
            update_edge_orien(&cube->edges[i], orien_fb, 1 ^ orien_fb, 1 ^ orien_fb);
        }
        else if ((e_slice(i) == M && e_slice(perm) == S) || (e_slice(i) == S && e_slice(perm) == M))
        {
            update_edge_orien(&cube->edges[i], orien_fb, orien_fb, 1 ^ orien_fb);
        }
        else if ((e_slice(i) == E && e_slice(perm) == S) || (e_slice(i) == S && e_slice(perm) == E))
        {
...
int which_corner_at_pos(int pos, cube_t* cube) {
    ...
    {
        if (extract_corner_perm(cube->corners[k]) == pos)
            return k;
    }
    return -1;
}
...
int which_edge_at_pos(int pos, cube_t* cube) {
    ...
    {
        if (extract_edge_perm(cube->edges[k]) == pos)
            return k;
    }
    return -1;
}
