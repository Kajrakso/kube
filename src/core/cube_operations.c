...
bool cube_operation_is_equal(cube_t cube1, cube_t cube2) {
  return arr_eq(cube1.edges, cube2.edges, NEDGES) &&
         arr_eq(cube1.corners, cube2.corners, NCORNERS);
}
...
cube_t cube_operation_compose(cube_t cube1, cube_t cube2) {
  ...
  for (int i = 0; i < NEDGES; i++) {
    ...
    result.edges[i] =
        build_edge(i2, extract_edge_orien(e1, FB) ^ extract_edge_orien(e2, FB),
  ...
  for (int i = 0; i < NCORNERS; i++) {
    ...
    result.corners[i] = build_corner(
        i2, (extract_corner_orien(c1, FB) + extract_corner_orien(c2, FB)) % 3,
...
cube_t cube_operation_inverse(cube_t cube) {
  ...
  for (i = 0; i < NEDGES; i++) {
    ...
    inverse.edges[extract_edge_perm(cube.edges[i])] =
        build_edge(i, eo_fb, eo_lr, eo_ud);
  }
  ...
  for (i = 0; i < NCORNERS; i++) {
    ...
    inverse.corners[extract_corner_perm(cube.corners[i])] =
        build_corner(i, co_fb, co_lr, co_ud);
  }
