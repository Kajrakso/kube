...
void cube_move_apply_multiple_moves(cube_t* cube, int* moves_arr,
...
static const int corner_perm_cycles[NFACES][4] = {
    [U] = {UBL, UBR, UFR, UFL}, [D] = {DFL, DFR, DBR, DBL},
    [L] = {UBL, UFL, DFL, DBL}, [R] = {UFR, UBR, DBR, DFR},
    [F] = {UFL, UFR, DFR, DFL}, [B] = {UBR, UBL, DBL, DBR},
};
  ...
  for (int co_fb = 0; co_fb < 3; co_fb++) {
    for (int co_lr = 0; co_lr < 3; co_lr++) {
      for (int co_ud = 0; co_ud < 3; co_ud++) {
        ...
        int new_co_fb =
            (is_quarter_move)
                ? (co_fb + corner_orient_change[FB][face][piece]) % 3
                : co_fb;
        int new_co_lr =
            (is_quarter_move)
                ? (co_lr + corner_orient_change[LR][face][piece]) % 3
                : co_lr;
        int new_co_ud =
            (is_quarter_move)
                ? (co_ud + corner_orient_change[UD][face][piece]) % 3
                : co_ud;
  ...
  for (int eo_fb = 0; eo_fb < 2; eo_fb++) {
    for (int eo_lr = 0; eo_lr < 2; eo_lr++) {
      for (int eo_ud = 0; eo_ud < 2; eo_ud++) {
        ...
        int new_eo_fb =
            (is_quarter_move) ? (edge_orient_change[FB][face] ^ eo_fb) : eo_fb;
        int new_eo_lr =
            (is_quarter_move) ? (edge_orient_change[LR][face] ^ eo_lr) : eo_lr;
        int new_eo_ud =
            (is_quarter_move) ? (edge_orient_change[UD][face] ^ eo_ud) : eo_ud;
