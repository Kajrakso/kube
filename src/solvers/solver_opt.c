...
static void print_stats(struct stats *stats) {
  if (stats->no_nodes_visited == 0) {
    ...
    fprintf(stderr, "inverse computations: %i (%.2f%%)\n",
            ...
            100 * (float)stats->no_inverse_computations /
                (float)stats->no_nodes_visited);
...
static inline bool prune(uint8_t x1, uint8_t x2, uint8_t x3, int rem_moves) {
  return ((x1 == rem_moves) && (x1 == x2) && (x2 == x3)) || (x1 > rem_moves) ||
         (x2 > rem_moves) || (x3 > rem_moves);
}

static bool TreeSearch(cube_t *cube, uint8_t *ptable, int remaining_moves,
                       struct stats *stats, int prev_move, int prev_move_inv,
                       bool is_inv, int max_num_sols) {
  stats->no_nodes_visited++;
    ...
    if (cube_state_is_solved(cube)) {
      ...
      for (int m = 0; m < 20; m++) {
        if (0 <= stats->solution[m] && stats->solution[m] < NMOVES) {
          stats->solutions[stats->num_sol_found * 20 + i++] =
              stats->solution[m];
        }
      }
      for (int m = 19; m >= 0; m--) {
        if (0 <= stats->solution_inv[m] && stats->solution_inv[m] < 18) {
          stats->solutions[stats->num_sol_found * 20 + i++] =
              get_inv_move(stats->solution_inv[m]);
        }
      }
      stats->num_sol_found++;
  ...
  for (int move = 0; move < NMOVES; move++) {
    ...
    if (!is_inv)
      stats->solution[20 - remaining_moves] = move;
    else
      stats->solution_inv[20 - remaining_moves] = move;

    bool found =
        TreeSearch(cube, ptable, remaining_moves - 1, stats,
    ...
    if (!is_inv)
      stats->solution[20 - remaining_moves] = -1;
    else
      stats->solution_inv[20 - remaining_moves] = -1;
...
void IDA(cube_t cube, int MAX_DEPTH, uint8_t *ptable, struct stats *stats,
         int max_num_sols) {
...
void solve_cube(cube_t cube, int max_depth, uint8_t *ptable, int max_num_sols,
  ...
  // actually search.
  IDA(cube, max_depth, ptable, stats, max_num_sols);

  print_stats(stats);

  memcpy(solutions, stats->solutions, sizeof(int) * 20 * max_num_sols);
...
bool cube_solvers_solve_cube(cube_t cube, int *solutions,
                             int number_of_solutions, int *banned_moves,
                             int num_banned_moves) {
