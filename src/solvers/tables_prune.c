...
void DLS_H(uint64_t ece, uint64_t eofb, uint64_t coud, uint64_t ccu, uint64_t p,
           int remaining_moves, int prev_move, int num_moves_done,
           uint8_t* ptable, uint64_t* sym_table_e_index) {
  ...
  for (int m = 0; m < NMOVES; m++) {
    ...
    DLS_H(ece2, eofb2, coud2, ccu2, p2, remaining_moves - 1, m,
          num_moves_done + 1, ptable, sym_table_e_index);
  }
}
...
void gen_ptable_H() {
  ...
  fprintf(
      stderr,
      "Search from nbhs. TODO: For whcih depth should we start doing this?\n");
